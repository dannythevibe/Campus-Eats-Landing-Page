...
function SignUpContent() {
  ...
  const role = searchParams.get("role") || "customer" // Get role from URL
      ...
      <div className="relative w-full max-w-md">
        <div className="bg-white/10 backdrop-blur-xl rounded-3xl p-8 shadow-2xl border border-white/20">
          ...
          <p className="text-center text-white/80 text-sm mt-6">
            Have an account?{" "}
            <Link href={`/auth/login?role=${role}`} className="text-orange-500 hover:text-orange-400 font-semibold">
              Log In
            </Link>
          </p>
        </div>
...
export default function SignUpPage() {
  return (
    <Suspense fallback={<div className="min-h-screen relative flex items-center justify-center p-6 text-white text-xl">Loading...</div>}>
      <SignUpContent />
    </Suspense>
  )
}
